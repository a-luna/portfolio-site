...
function find_cusp(a: number, b: number): [number, number] {
	// First, find the maximum saturation (saturation S = C/L)
	const S_cusp = compute_max_saturation(a, b);

	// Convert to linear sRGB to find the first point where at least one of r,g or b >= 1:
	const rgb = oklab_to_linear_srgb({ l: 1, a: S_cusp * a, b: S_cusp * b, A: 1 });
	const L_cusp = Math.cbrt(1 / Math.max(Math.max(rgb.r, rgb.g), rgb.b));
	const C_cusp = L_cusp * S_cusp;

	return [L_cusp, C_cusp];
}

export function oklab_to_linear_srgb(oklab: OklabColor): RgbColor {
	const { l: L, a, b, A } = oklab;
...
function get_ST_max(a_: number, b_: number, cusp: [number, number] | null = null): [number, number] {
	if (!cusp) {
